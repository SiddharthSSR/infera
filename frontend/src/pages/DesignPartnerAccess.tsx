...
export function DesignPartnerAccess({ endpoint = designPartnerRequestEndpoint }: DesignPartnerAccessProps) {
  ...
  return (
    <AppShell variant="public" className="request-access-shell">
      ...
      <PublicNav title="DESIGN-PARTNER ACCESS" />

      <main id="main-content">
        <header className="request-access-header">
          <div>
            <span className="landing-meta">Evaluation / design partner</span>
            <h1>Bring one inference problem. Evaluate it with us.</h1>
          </div>
          <div className="request-access-intro">
            <p>Tell us where your current inference path is getting in the way and what you need to verify. This is a request for a design-partner conversation, not a paid subscription or a promise of access.</p>
        ...
        <section className="request-access-layout" aria-labelledby="request-form-heading">
          <aside className="request-access-steps" aria-label="What happens next">
            <span className="landing-meta">The path</span>
            <ol>
          ...
          <div className="request-access-form-panel">
            {submitState === 'succeeded' ? (
              <div className="request-access-success" role="status" tabIndex={-1}>
              ...
              <form noValidate aria-busy={submitState === 'submitting'} onSubmit={(event) => void handleSubmit(event)} onFocusCapture={recordStart}>
                ...
                {!endpoint ? (
                  <div className="request-access-config" role="status">
                    Request delivery is not configured yet. An administrator must connect an approved secure intake endpoint before submissions can be delivered.
                  </div>
                ) : null}

...
class WorkerConfig(BaseSettings):
    ...
    worker_address: str = Field(
        default="", description="Public address of this worker (for registration)"
    )
    ...
    drain_timeout_s: int = Field(
        default=30,
        description="Seconds to wait for in-flight requests to finish on graceful shutdown",
    )
    ...
    vllm_enable_prefix_caching: bool = Field(
        default=True, description="Enable automatic prefix caching (KV cache reuse)"
    )
    vllm_enable_chunked_prefill: bool = Field(
        default=True, description="Enable chunked prefill to avoid blocking decode batches"
    )
    vllm_max_num_batched_tokens: int | None = Field(
        default=None, description="Max tokens per prefill chunk (None = vLLM default)"
    )
    vllm_max_num_seqs: int | None = Field(
        default=None, description="Max concurrent sequences per iteration (None = vLLM default)"
    )
    vllm_swap_space: float | None = Field(
        default=None, description="CPU swap space for KV spill in GiB (None = vLLM default)"
    )
    vllm_enforce_eager: bool = Field(
        default=False, description="Disable CUDA graphs and force eager execution"
    )
    vllm_num_scheduler_steps: int = Field(
        default=0, description="Multi-step scheduling steps per iteration (0 = disabled)"
    )
    ...
    vllm_speculative_model: str = Field(
        default="", description="Draft model ID or '[ngram]' for speculative decoding"
    )
    vllm_num_speculative_tokens: int = Field(
        default=0, description="Tokens to speculate per step (0 = disabled)"
    )
    vllm_ngram_prompt_lookup_num_tokens: int = Field(
        default=0, description="Ngram look-back window (ngram mode only)"
    )
    ...
    sglang_chunked_prefill_size: int | None = Field(
        default=None, description="Chunked prefill size"
    )
    sglang_max_running_requests: int | None = Field(
        default=None, description="Max running requests"
    )
    ...
    tensorrt_llm_max_num_tokens: int | None = Field(
        default=None, description="Max tokens per iteration"
    )

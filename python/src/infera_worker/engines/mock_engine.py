"""Mock inference engine for tests and local development."""
...
class MockEngine(BaseInferenceEngine):
    ...
    async def infer_stream(self, request: InferenceRequest):
        self.active_requests.add(request.request_id)
...
def create_mock_engine(config: WorkerConfig):
...
register_engine(
    EngineDefinition(
        engine_id="mock",
        display_name="Mock",
        create=create_mock_engine,
        optional_dependency=None,
        capabilities=EngineCapabilities(
            supports_streaming=True,
            supports_cancellation=True,
            supports_dynamic_model_loading=True,
            supports_runtime_warmup=False,
        ),
    )
)

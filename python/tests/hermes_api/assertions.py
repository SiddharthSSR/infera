...
def assert_status_code(result: APICallResult[Any], expected: int) -> None:
    assert result.status_code == expected, f"expected HTTP {expected}, got {result.status_code}: {result.text}"
...
def assert_error_type(result: APICallResult[Any], expected: str) -> None:
    ...
    assert payload.error.type == expected, f"expected error type {expected!r}, got {payload.error.type!r}"
...
def assert_argument_subset(actual: Any, expected: Any, detail: RunDetailResponse) -> None:
    ...
    if isinstance(expected, list):
        ...
        assert len(actual) >= len(expected), f"expected list of at least {len(expected)}, got {len(actual)}"
        for actual_item, expected_item in zip(actual, expected, strict=False):
            assert_argument_subset(actual_item, expected_item, detail)
        return
    assert actual == _normalize_expected_argument(expected, detail), f"expected argument {expected!r}, got {actual!r}"
...
def assert_scenario_tools(detail: RunDetailResponse, case: ScenarioCase) -> None:
    ...
    for tool_name, expected_arguments in case.expected_arguments.items():
        matching_arguments = [arguments for actual_tool_name, arguments in calls if actual_tool_name == tool_name]
        assert matching_arguments, f"expected arguments for tool {tool_name!r}, but no tool call was recorded"
...
def assert_final_output_matches(detail: RunDetailResponse, expected_keywords: list[str]) -> None:
    ...
    assert not missing, f"final_output is missing expected keywords {missing!r}: {detail.run.final_output!r}"


def assert_terminal_status(detail: RunDetailResponse, expected_status: str) -> None:
    assert detail.run.status == expected_status, f"expected terminal status {expected_status!r}, got {detail.run.status!r}"
...
def assert_schema_coherent(detail: RunDetailResponse) -> None:
    ...
    if detail.sources:
        assert any(step.tool_name == "web_search" for step in detail.steps), "sources should come from web_search"


def mark_mock_only_if_live(case_id: str, is_live: bool) -> None:
    if is_live:
        pytest.skip(f"{case_id} is a deterministic mock scenario; use the live smoke tests for deployed Hermes")

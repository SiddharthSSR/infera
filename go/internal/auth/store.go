...
var authMigrations = []migrate.Migration{
	...
	{
		Version:     9,
		Description: "bind workspace memberships to immutable human identities",
		SQL: `
		ALTER TABLE workspace_memberships ADD COLUMN human_identity_id TEXT;
...
func newStore(dbPath string, credentialCipher *providerCredentialCipher) (*Store, error) {
	// Immediate transactions acquire SQLite's database-wide writer boundary
	// before lifecycle reads. This serializes multi-statement mutations across
	// independent Store instances and gateway processes.
...
func (s *Store) CreateKeyWithPrincipalInWorkspace(workspaceID, name, role, principalType string) (string, *KeyRecord, error) {
	...
	result, err := s.db.Exec(`
		...
		SELECT ?, w.id, ?, ?, ?, ?, ?, NULL, ?, 'active'
		FROM workspaces w
		WHERE w.id = ? AND w.status = ?`,
		id, hash, prefix, name, role, principalType, now, workspace.ID, WorkspaceStatusActive,
	)
	if err != nil {
	...
	if err != nil {
		return "", nil, fmt.Errorf("failed to confirm key creation: %w", err)
	}
	if rows != 1 {
		return "", nil, fmt.Errorf("active workspace is required")
	}
...
func (s *Store) ValidateKey(rawKey string) (*KeyRecord, error) {
	...
	row := s.db.QueryRow(`
		SELECT k.id, k.workspace_id, w.slug, w.name, k.key_prefix, k.name,
		       COALESCE(m.role, k.role), k.principal_type, k.membership_id, m.human_identity_id, m.email, m.display_name,
	...
	err := row.Scan(
		...
		&record.KeyPrefix, &record.Name, &record.Role, &record.PrincipalType, &record.MembershipID, &record.HumanIdentityID, &record.MemberEmail, &record.MemberName,
...
func (s *Store) ValidateSession(rawToken string) (*SessionRecord, *KeyRecord, error) {
	...
	err := row.Scan(
		...
		&key.ID, &key.WorkspaceID, &key.WorkspaceSlug, &key.WorkspaceName, &key.KeyPrefix, &key.Name, &key.Role, &key.PrincipalType, &key.MembershipID, &key.HumanIdentityID, &key.MemberEmail, &key.MemberName, &key.CreatedAt, &key.LastUsed, &key.Status,
	)
	if err != nil {
		if err == sql.ErrNoRows {
...
func (s *Store) DeactivateWorkspace(workspaceID string) (*WorkspaceRecord, error) {
	...
	workspace, err := s.deactivateWorkspaceTx(tx, workspaceID)
	if err != nil {
		return nil, err
	}
	if err := tx.Commit(); err != nil {
...
func (s *Store) deactivateWorkspaceTx(tx *sql.Tx, workspaceID string) (*WorkspaceRecord, error) {
...
func (s *Store) ListAccessibleWorkspaces(record *KeyRecord) ([]*WorkspaceRecord, error) {
	...
	if record.PrincipalType == PrincipalHuman && record.HumanIdentityID != nil {
		identityID := strings.TrimSpace(*record.HumanIdentityID)
		if identityID != "" {
			rows, err := s.db.Query(`
...
func (s *Store) SwitchSessionWorkspace(rawToken, workspaceID string) (*SessionRecord, *KeyRecord, error) {
	...
	if currentKey.HumanIdentityID == nil || strings.TrimSpace(*currentKey.HumanIdentityID) == "" {
		return nil, nil, fmt.Errorf("workspace switching is unavailable for this session")
	}

	targetKeyID, err := s.findSwitchableWorkspaceKeyID(*currentKey.HumanIdentityID, workspaceID)
	if err != nil {
		return nil, nil, err
	}
	...
	result, err := s.db.Exec(`
		...
		targetKeyID, time.Now(), session.ID,
		targetKeyID, workspaceID, WorkspaceStatusActive,
	)
	if err != nil {
	...
	rows, err := result.RowsAffected()
	if err != nil {
		return nil, nil, fmt.Errorf("failed to confirm session workspace switch: %w", err)
	}
	if rows != 1 {
		return nil, nil, fmt.Errorf("session or target workspace is no longer active")
	}

	return s.ValidateSession(rawToken)
}

func (s *Store) findSwitchableWorkspaceKeyID(identityID, workspaceID string) (string, error) {
	identityID = strings.TrimSpace(identityID)
	workspaceID = strings.TrimSpace(workspaceID)
	if identityID == "" || workspaceID == "" {
		return "", fmt.Errorf("human identity and workspace id are required")
	}
...
func (s *Store) UpsertWorkspaceQuota(workspaceID string, monthlyRequestLimit, monthlyTokenLimit *int64, enforceHardLimits bool) (*WorkspaceQuotaRecord, error) {
	...
	rows, err := result.RowsAffected()
	if err != nil {
		return nil, fmt.Errorf("failed to confirm workspace quota update: %w", err)
	}
	if rows != 1 {
		return nil, fmt.Errorf("active workspace is required")
	}
...
func (s *Store) UpsertWorkspaceProviderConfig(workspaceID, provider, apiKey, apiSecret, endpoint string, options map[string]string) (*WorkspaceProviderConfigRecord, error) {
	...
	rows, err := result.RowsAffected()
	if err != nil {
		return nil, fmt.Errorf("failed to confirm workspace provider config update: %w", err)
	}
	if rows != 1 {
		return nil, fmt.Errorf("active workspace is required")
	}
...
func (s *Store) ListWorkspaceMemberships(workspaceID string) ([]*WorkspaceMembershipRecord, error) {
	...
	rows, err := s.db.Query(`
		SELECT id, workspace_id, human_identity_id, email, display_name, role, status, created_at
	...
	for rows.Next() {
		...
		if err := rows.Scan(&m.ID, &m.WorkspaceID, &m.HumanIdentityID, &m.Email, &m.DisplayName, &m.Role, &m.Status, &m.CreatedAt); err != nil {
			return nil, err
		}
		out = append(out, m)
	}
...
func (s *Store) GetWorkspaceMembership(workspaceID, membershipID string) (*WorkspaceMembershipRecord, error) {
	...
	m := &WorkspaceMembershipRecord{}
	if err := row.Scan(&m.ID, &m.WorkspaceID, &m.HumanIdentityID, &m.Email, &m.DisplayName, &m.Role, &m.Status, &m.CreatedAt); err != nil {
		if err == sql.ErrNoRows {
...
func (s *Store) CreateWorkspaceInvitation(workspaceID, email, displayName, role, invitedByKeyID string, expiresAt time.Time) (string, *WorkspaceInvitationRecord, error) {
	...
	result, err := s.db.Exec(`
		...
		id, email, displayName, role, tokenHash, invitedByKeyID, now, expiresAt,
		workspaceID, WorkspaceStatusActive, email,
	)
	if err != nil {
	...
	rows, err := result.RowsAffected()
	if err != nil {
		return "", nil, fmt.Errorf("failed to confirm workspace invitation creation: %w", err)
	}
	if rows != 1 {
		return "", nil, fmt.Errorf("active workspace without an existing membership is required")
	}
...
func (s *Store) AcceptWorkspaceInvitation(rawToken, displayName string) (*WorkspaceMembershipRecord, string, *KeyRecord, error) {
	return s.acceptWorkspaceInvitation(rawToken, displayName, "")
}

// AcceptWorkspaceInvitationForIdentity accepts an invitation on behalf of an
// already-authenticated human identity. The identity value must come from a
// validated key or session, never from invitation input.
func (s *Store) AcceptWorkspaceInvitationForIdentity(rawToken, displayName string, identity *KeyRecord) (*WorkspaceMembershipRecord, string, *KeyRecord, error) {
	if identity == nil || identity.Status != "active" || identity.PrincipalType != PrincipalHuman || identity.HumanIdentityID == nil {
		return nil, "", nil, fmt.Errorf("verified human identity is required")
	}
	humanIdentityID := strings.TrimSpace(*identity.HumanIdentityID)
	if humanIdentityID == "" {
		return nil, "", nil, fmt.Errorf("verified human identity is required")
	}
	return s.acceptWorkspaceInvitation(rawToken, displayName, humanIdentityID)
}
...
func (s *Store) acceptWorkspaceInvitation(rawToken, displayName, humanIdentityID string) (*WorkspaceMembershipRecord, string, *KeyRecord, error) {
	...
	if humanIdentityID == "" {
		humanIdentityID = "usr_" + uuid.New().String()
	}
	...
	err = tx.QueryRow(`
		SELECT wi.id, wi.workspace_id, wi.email, wi.display_name, wi.role, wi.status, wi.expires_at,
		       w.slug, w.name, w.created_at, w.status
		FROM workspace_invitations wi
		JOIN workspaces w ON w.id = wi.workspace_id
		WHERE wi.invite_token_hash = ?`,
		tokenHash,
	...
	if workspace.Status != WorkspaceStatusActive {
		return nil, "", nil, fmt.Errorf("active workspace is required")
	}
	...
	var existing int
	if err := tx.QueryRow(`SELECT COUNT(1) FROM workspace_memberships WHERE workspace_id = ? AND (email = ? OR human_identity_id = ?) AND status = 'active'`, invite.WorkspaceID, invite.Email, humanIdentityID).Scan(&existing); err != nil {
	...
	rows, err := result.RowsAffected()
	if err != nil {
		return nil, "", nil, fmt.Errorf("failed to confirm invitation status update: %w", err)
	}
	if rows != 1 {
		return nil, "", nil, fmt.Errorf("invitation is no longer pending")
	}
